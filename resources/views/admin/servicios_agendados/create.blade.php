...
@section('content')
    ...
    <form action="{{ route('servicios_agendados.store') }}" method="POST" class="row" id="form-services"
        enctype="multipart/form-data">
        @csrf
        @include('admin.servicios_agendados.form', ['servicio' => null])
    </form>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/dropzone/5.9.3/min/dropzone.min.js"></script>
    <script>
        Dropzone.autoDiscover = false;
        const myDropzone = new Dropzone("#registro-fotografico-dropzone", {
            url: "{{ route('servicios_agendados.store') }}", // Mismo endpoint que el formulario
